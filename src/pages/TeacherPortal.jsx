...
import { 
  ...
  Gamepad2, Calendar, CheckCircle2, MessageSquare, Star, RefreshCw, FileSpreadsheet, FolderOpen, FileText 
} from 'lucide-react';
...
import { MaterialFolderTree } from '../components/common/MaterialFolderTree';
...
export const TeacherPortal = () => {
  ...
  const [selectedImportClass, setSelectedImportClass] = useState(null);
  ...
  const handleSelectTreeMaterial = (mat) => {
    if (mat.type === 'game_html5') {
      setActiveGame(mat);
    } else {
      setActiveViewerDoc(mat);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F4FAF8]">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full space-y-10">
        ...
        <div className="bg-edulive-gradient rounded-3xl p-8 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-sm text-teal-50 max-w-xl">
              Tạo lớp học mới tự sinh Mã QR Code, Import học sinh từ Excel, tra cứu Cây thư mục học liệu và giao bài tập.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => { setSelectedImportClass(classes[0]); setImportModalOpen(true); }}
              className="bg-white/20 hover:bg-white/30 text-white font-extrabold text-sm px-5 py-3 rounded-full border border-white/40 shadow-sm flex items-center gap-2 transition-all"
            >
            ...
            <button
              ...
              className="btn-eduyellow font-black text-sm px-6 py-3 rounded-full shadow-lg transform hover:scale-105"
            >
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => { setSelectedImportClass(classes[0]); setImportModalOpen(true); }}
                className="bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-xs py-2 px-4 rounded-full border border-amber-300 flex items-center gap-1.5"
              >
                <FileSpreadsheet className="w-4 h-4 text-amber-700" /> Import Excel
              </button>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => { setSelectedImportClass(cls); setImportModalOpen(true); }}
                        className="p-1.5 bg-teal-50 hover:bg-teal-100 text-teal-800 rounded-lg text-xs font-bold"
                        title="Import Học sinh vào lớp này"
                      >
                        <FileSpreadsheet className="w-4 h-4 text-[#36b3a0]" />
                      </button>
                      <button
        ...
        {/* SECTION 2: CÂY THƯ MỤC HỌC LIỆU TAXONOMY */}
        <MaterialFolderTree 
          onSelectMaterial={handleSelectTreeMaterial} 
          onAssignMaterial={handleAssignGame} 
        />
      ...
      <ImportStudentsModal
        ...
        targetClass={selectedImportClass}
        onImportSuccess={(importedList) => {
          alert(`Đã import thành công ${importedList.length} học sinh!`);
        }}
      />
